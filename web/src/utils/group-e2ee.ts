...
export type GroupMemberPublicKey = {
  userId: number
  publicKey: string
  publicKeyAlgorithm: string
}

export type GroupMessageMemberKey = {
  userId: number
...
export type GroupMessageEnvelope = {
  ...
  memberKeys: GroupMessageMemberKey[]
}
...
async function importAesKeyRaw(raw: ArrayBuffer) {
 ...
 */
export async function buildGroupMessageEnvelope(
  plaintext: string,
  members: GroupMemberPublicKey[]
  ...
  const memberKeys: GroupMessageMemberKey[] = []
  for (const m of members) {
...
export async function decryptGroupMessage(
  ...
  const plain = await window.crypto.subtle.decrypt(
    { name: 'AES-GCM', iv: new Uint8Array(base64ToBytes(contentIv)) },
    aesKey,
    base64ToBytes(contentCiphertext)
  )

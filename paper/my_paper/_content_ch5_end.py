...
def write_end(doc):
    ...
    add_para(doc, '本文围绕“如何在前后端分离的 IM 系统中引入可落地的消息加密机制”这一问题，'
                  ...
                  '一对一即时通讯场景下的完整业务闭环。')
    add_para(doc, '从工程实现角度看，本文的工作并不止于“做出一个能聊天的系统”。'
                  '一方面，系统通过分层架构将路由、中间件、业务逻辑、数据访问与连接管理相互解耦，'
                  '保证了整体结构的清晰性与后续扩展的可行性；'
                  '另一方面，系统在实时通信链路上采用“HTTP 持久化通道 + WebSocket 实时通道”的协同方式，'
                  '在保证消息可靠落库的同时，实现了低延迟的实时推送。测试结果表明，'
                  '系统在功能完整性、交互流程与运行稳定性方面均达到了预期目标。')
    add_para(doc, '从安全模型角度看，本文最核心的工作在于将端到端消息加密机制落入具体实现。'
                  '系统采用 RSA-OAEP 与 Web Crypto API，由浏览器本地生成并保存私钥，'
                  '发送阶段对同一条明文构造发送方密文与接收方密文两份副本，服务端仅负责密文的存储与转发。'
                  '这一设计在保证聊天双方都可本地查看历史消息的同时，'
                  '有效缩小了服务端的明文暴露边界。换言之，'
                  '即便服务端数据库被直接获取，攻击者所能拿到的也只是密文而非可读消息内容。'
                  '这正是本文区别于普通课程项目型 IM 系统的关键所在。')
    add_para(doc, '需要指出的是，本文实现的方案仍然属于“前端本地加密 + 服务端密文转发与存储”模式，'
                  '尚未达到工业级完整 E2EE 体系的严格要求。公钥真实性校验缺失意味着在极端威胁模型下'
                  '仍可能受到中间人攻击；长期 RSA 私钥直接参与消息解密，也意味着系统尚不具备前向保密能力；'
                  '同时，会话密钥协商、多设备密钥同步以及群聊场景下的密钥管理问题都还没有纳入当前实现。'
                  '这些问题并非被忽视，而是出于毕业设计场景下对实现复杂度、开发周期与展示重点的综合权衡而有意留白。')
    add_para(doc, '后续工作可以沿三个方向继续推进。其一，引入公钥指纹比对、可信目录或带外验证机制，'
                  '建立更可靠的公钥真实性保障；其二，将当前的 RSA 直接加密方案演进为'
                  '“非对称密钥协商 + 对称密钥加密正文”的混合机制，并在此基础上进一步引入前向保密能力；'
                  '其三，在现有一对一聊天模型上扩展群聊、离线消息、已读回执、多设备同步等典型 IM 功能。'
                  '这些演进路径表明，本文工作不仅完成了一个可运行的毕业设计系统，'
                  '也为进一步向更高安全等级和更强工程能力的即时通讯平台演化奠定了基础。')
    add_page_break(doc)
